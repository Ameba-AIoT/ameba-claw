-- test_touch.lua
--
-- TOUCH_TEST_MODE controls which test case runs:
--   "interactive" -- wait for physical finger touch on PA_17 (default)
--   "ext"         -- raw data monitor; no touch required; tests init + baseline
--
-- Run via: AT+CLAW=touch        (interactive)
--          AT+CLAW=touch,ext    (ext raw monitor)
--
-- TOUCH_TEST_MODE is injected as a Lua global by the C provision layer.

local TEST_MODE = TOUCH_TEST_MODE or "interactive"
...
local function run_interactive()
    ...
    print("[touch_test] init CapTouch on " .. PIN1 .. " (CH4) + " .. PIN2 .. " (CH3)")

    -- CH4 (PA_16, key index 1): mbias=0x590, threshold=80
    -- CH3 (PA_17, key index 2): mbias=0x560, threshold=50
    local ok, dev = pcall(touch.new, PIN1, PIN2,
                          {n_noise_thr = 40, p_noise_thr = 40,
                           interval_ms = 10,
                           ch_mbias     = {0x590, 0x560},
                           ch_threshold = {80,    50}})
    if not ok then
        print("[touch_test] FAIL: touch.new failed: " .. tostring(dev))
        return false
    end
    print("[touch_test] opened: " .. dev:name())

    print("[touch_test] waiting for baseline calibration (1 s)...")
    sys.sleep_ms(1000)
    ...
    print(string.format(
        "[touch_test] please press and release BOTH %s and %s within %d s",
        PIN1, PIN2, TIMEOUT_MS // 1000))

    while elapsed_ms < TIMEOUT_MS do
        local ok_r, sample = pcall(dev.read, dev)
        if not ok_r then
            print("[touch_test] FAIL: read error: " .. tostring(sample))
            dev:close()
            return false
        end
        ...
        for _, key in ipairs(sample.keys) do
            ...
            if key.pressed ~= last_state[i] then
                print(string.format(
                    "[touch_test] %s  pin=%s ch=%d smooth=%d benchmark=%d delta=%d",
                ...
                elseif pressed_done[i] then
                    ...
                    print(string.format("[touch_test] %s press+release OK", PIN_NAME[i]))
                end
    ...
    if pass then
        print("[touch_test] PASS: both channels press+release detected")
    else
        for i = 1, 2 do
            if not released_done[i] then
                print(string.format("[touch_test] FAIL: %s did not complete press+release", PIN_NAME[i]))
            end
        end
    end
    return pass
...
local function run_ext()
    ...
    print("[touch_test] === ext raw-monitor test on " .. PIN1 .. " + " .. PIN2 .. " ===")
    print(string.format(
        "[touch_test] reading %d samples every %d ms (no touch needed)",
        SAMPLES, INTERVAL_MS))
    ...
    for i = 1, SAMPLES do
        ...
        for _, key in ipairs(sample.keys) do
            ...
            print(string.format(
                "[touch_test] sample %2d key%d(%s ch%d): smooth=%5d benchmark=%5d delta=%5d  [%s]",
            ...
            if ok_cs then
                print(string.format("[touch_test]   key%d hw_status=0x%x", key.index, cst))
            end
        end
...
local function main()
    ...
    if TEST_MODE == "interactive" then
        print("[touch_test] mode=interactive  (touch PA17 with finger)")
        pass = run_interactive()
    elseif TEST_MODE == "ext" then
        print("[touch_test] mode=ext  (raw monitor, no touch needed)")
        pass = run_ext()
    else
        print("[touch_test] ERROR: unknown TEST_MODE: " .. tostring(TEST_MODE))
        return
    end

    if pass then
        print("[touch_test] success")
    else
        print("[touch_test] FAIL")
    end
end

local ok, err = pcall(main)
if not ok then
    print("[touch_test] ERROR: " .. tostring(err))
end
