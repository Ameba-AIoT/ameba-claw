...
#define LUA_DRIVER_TOUCH_DEFAULT_THRESHOLD   1600u
#define LUA_DRIVER_TOUCH_DEFAULT_MBIAS       0x800u
#define LUA_DRIVER_TOUCH_DEFAULT_NOISE_THR   800u
/* Default scan interval: 60 counts * (1/1.024 kHz) ≈ 58.6 ms */
#define LUA_DRIVER_TOUCH_DEFAULT_INTERVAL_MS 60u

typedef struct {
    u8  channel[LUA_DRIVER_TOUCH_MAX_KEYS];
    u8  pin[LUA_DRIVER_TOUCH_MAX_KEYS];
    u32 threshold[LUA_DRIVER_TOUCH_MAX_KEYS];
    int key_count;
    char name[LUA_DRIVER_TOUCH_MAX_NAME_LEN];
    int closed;
...
{
    lua_driver_touch_ud_t *ud = (lua_driver_touch_ud_t *)luaL_checkudata(
        L, idx, LUA_DRIVER_TOUCH_METATABLE);
    if (!ud || ud->closed) {
...
static void lua_driver_touch_enable_channel(u8 ch, u32 threshold,
...
static int lua_driver_touch_new(lua_State *L)
{
     ...
     *   touch.new(pin)
     *   touch.new(pin1, pin2)
     *   touch.new(pin, {threshold=N, name="..."})
     *   touch.new(pin1, pin2, {threshold=N, name="..."}) */

    PinName pins[LUA_DRIVER_TOUCH_MAX_KEYS];
    int key_count = 0;
    u32 threshold   = LUA_DRIVER_TOUCH_DEFAULT_THRESHOLD;
    u16 mbias       = (u16)LUA_DRIVER_TOUCH_DEFAULT_MBIAS;
    u16 n_noise_thr = (u16)LUA_DRIVER_TOUCH_DEFAULT_NOISE_THR;
    u16 p_noise_thr = (u16)LUA_DRIVER_TOUCH_DEFAULT_NOISE_THR;
    u32 interval_ms = LUA_DRIVER_TOUCH_DEFAULT_INTERVAL_MS;
    /* Per-channel overrides (0 means use common value) */
    u16 ch_mbias[LUA_DRIVER_TOUCH_MAX_KEYS]     = {0, 0};
    u32 ch_threshold[LUA_DRIVER_TOUCH_MAX_KEYS] = {0, 0};
    const char *dev_name = LUA_DRIVER_TOUCH_DEFAULT_NAME;
    ...
    if (arg2_type == LUA_TSTRING || arg2_type == LUA_TNUMBER) {
        if (key_count < LUA_DRIVER_TOUCH_MAX_KEYS) {
    ...
    if (lua_istable(L, next_arg)) {
        ...
        if (lua_isnumber(L, -1)) {
            ...
            if (t <= 0) {
                return luaL_error(L, "touch: threshold must be positive");
            }
            threshold = (u32)t;
        }
        lua_pop(L, 1);
        ...
        if (lua_istable(L, -1)) {
            for (int i = 0; i < LUA_DRIVER_TOUCH_MAX_KEYS; i++) {
                lua_rawgeti(L, -1, i + 1);
                if (lua_isnumber(L, -1)) {
    ...
    int channels[LUA_DRIVER_TOUCH_MAX_KEYS];
    for (int i = 0; i < key_count; i++) {
        channels[i] = pin_to_ctc_channel((u8)pins[i]);
        if (channels[i] < 0) {
            return luaL_error(L, "touch: pin 0x%02x is not a CapTouch-capable pin", (u8)pins[i]);
        }
    }
    ...
    if (!s_ctc_initialized) {
        ...
        for (int i = 0; i < key_count; i++) {
            ...
            lua_driver_touch_enable_channel((u8)channels[i], ch_thr, ch_mb, n_noise_thr, p_noise_thr);
        }
    }
    s_ctc_refcount++;
    ...
    /* Create Lua userdata */
    lua_driver_touch_ud_t *ud = (lua_driver_touch_ud_t *)lua_newuserdata(
        L, sizeof(*ud));
    ...
    if (strlen(dev_name) < LUA_DRIVER_TOUCH_MAX_NAME_LEN) {
        strcpy(ud->name, dev_name);
    } else {
        strncpy(ud->name, dev_name, LUA_DRIVER_TOUCH_MAX_NAME_LEN - 1);
        ud->name[LUA_DRIVER_TOUCH_MAX_NAME_LEN - 1] = '\0';
    }
...
static void lua_driver_touch_push_key_table(lua_State *L, int index, int channel,
...
static int lua_driver_touch_read(lua_State *L)
{
    ...
    for (int i = 0; i < ud->key_count; i++) {
        ...
        lua_driver_touch_push_key_table(L, i, (int)ch, (int)ud->pin[i],
...
static int lua_driver_touch_set_threshold(lua_State *L)
{
    lua_driver_touch_ud_t *ud = lua_driver_touch_get_ud(L, 1);
    int index = (int)luaL_checkinteger(L, 2);
...
static int lua_driver_touch_set_mbias(lua_State *L)
{
    lua_driver_touch_ud_t *ud = lua_driver_touch_get_ud(L, 1);
    int index = (int)luaL_checkinteger(L, 2);
    lua_Integer val = luaL_checkinteger(L, 3);
...
static int lua_driver_touch_set_scan_interval(lua_State *L)
{
    lua_driver_touch_get_ud(L, 1);
...
static int lua_driver_touch_get_ch_status(lua_State *L)
{
    lua_driver_touch_ud_t *ud = lua_driver_touch_get_ud(L, 1);
    int index = (int)luaL_checkinteger(L, 2);
...
static void lua_driver_touch_do_close(lua_driver_touch_ud_t *ud)
{
    if (ud->closed) {
        return;
    }
...
{
    lua_driver_touch_ud_t *ud = (lua_driver_touch_ud_t *)luaL_checkudata(
        L, 1, LUA_DRIVER_TOUCH_METATABLE);
    lua_driver_touch_do_close(ud);
    return 0;
}

static int lua_driver_touch_gc(lua_State *L)
{
    lua_driver_touch_ud_t *ud = (lua_driver_touch_ud_t *)luaL_testudata(
        L, 1, LUA_DRIVER_TOUCH_METATABLE);
    if (ud) {
        lua_driver_touch_do_close(ud);
    }
...
int luaopen_touch(lua_State *L)
{
    ...
    if (luaL_newmetatable(L, LUA_DRIVER_TOUCH_METATABLE)) {
        ...
        lua_pushcfunction(L, lua_driver_touch_set_threshold);
        lua_setfield(L, -2, "set_threshold");
        lua_pushcfunction(L, lua_driver_touch_set_mbias);
        lua_setfield(L, -2, "set_mbias");
        lua_pushcfunction(L, lua_driver_touch_set_scan_interval);
        lua_setfield(L, -2, "set_scan_interval");
        lua_pushcfunction(L, lua_driver_touch_get_ch_status);
