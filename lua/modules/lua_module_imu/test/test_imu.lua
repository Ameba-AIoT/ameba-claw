-- test_imu.lua
-- MPU-6050 (GY-521) IMU test for AmebaGreen2 (RTL8721F).
--
-- Wiring (board.json i2c0 interface on EV721FL0_R03_BreadBoard):
--   VCC -> 3V3, GND -> GND, SCL -> PA_25, SDA -> PA_26, AD0 -> GND (addr 0x68)
--
-- Usage:
...
local function run()
    ...
    assert(id == 0x68, string.format("who_am_i() expected 0x68, got 0x%02x", id))
    ...
    print(string.format("[imu] read_int_status(): 0x%02x  OK", st))
    ...
    sys.sleep_ms(100)
    dev = imu.new({ sda = SDA, scl = SCL, i2c = I2C, addr = ADDR })
    local s2 = dev:read()
